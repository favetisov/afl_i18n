export function filter(items:Array<any>, query:string, fields?:Array<string>) {
  ...
  let checkWholeItem = function(item) {
    for (let q of queries) {
      if (!checkPart(q, item)) {return false} //каждая из частей запроса должна входить хотя бы в одну из частей элемента
    }
    return true;
  };
    ...
    if (fields) {
      for (let field of fields) {
        ...
        for (let f of field.split('.')) {
          if (!part[f]) { part = ''; break; } else { part = part[f]; }
        }
        parts.push(part);
      }
    } else {  //фильтруем массив простых элементов
      parts.push(item);
    }
    for (let p of parts) {
      if (simplifyString(p).indexOf(value) !== -1) {return true}
    }
